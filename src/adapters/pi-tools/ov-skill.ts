    ...
    async execute(_toolCallId, params, signal) {
      try {
        const data: string | SkillData = params.name || params.description
          ...
          : params.content!;
        const result = await withLogging<Awaited<ReturnType<SkillClient["addSkill"]>>>(logger, "ov_skill", async () => {
          const r = await client.addSkill(data, { wait: params.wait }, signal ?? undefined);
          return { value: r, extra: { name: r.name } };
        });
