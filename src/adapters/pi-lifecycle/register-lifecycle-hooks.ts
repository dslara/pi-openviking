...
import type { OVAdapter } from "../ov-client/adapter";
...
import { OVWidget } from "../ui/ov-widget";
...
import { SessionSync, DEFAULT_AUTO_COMMIT_INTERVAL_MS } from "../../domain/session/session-sync-service";
import { RecallCache } from "../../domain/common/recall-cache";
import { hashString, extractLatestUserText } from "./lifecycle-utils";
...
// helpers extracted to lifecycle-utils.ts
...
export async function handleSessionStart(
  ...
  if (profileManager && autoDetectRules) {
    const { autoDetectProfile } = await import("../../domain/profile/auto-detect");
